...
int load_elf_binary(struct m_inode *inode, struct buffer_head *bh, bin_section *bs)
{
        ...
        while (nsect--) {
                ...
                sect=*((Elf32_Shdr *)(bht->b_data + (ex.e_shoff + nsect * ex.e_shentsize) % BLOCK_SIZE));
                if (!sect.sh_size || !sect.sh_addr)
                        continue;
                
                switch (sect.sh_type) {
                        case SHT_PROGBITS:
                                if (copy_section(inode, sect.sh_offset, sect.sh_addr, sect.sh_size)) {
                                        if (bht)
                                                brelse(bht);
                                        return -1;
                                }
